...
void print_tree(NODE *root, const char *prefix, int is_left) {
   ...
   *params:
        ...
        prefix: the pattern to print, based on weather its the left or right branch 
        is_left: flag to check for the left branch
   */
...
void make_encoding_array(NODE *root, int len, char *buffer, int buf_len, char *codes[len], int depth) {
    /* this function makes the encoding array from the huffman tree, by following in inorder traversal method 
     * the algorithm is that any left branch taken will append 0 to the huffman code for a character, and a right
     * branch taken will append 1. This is tracked by the buffer, which is shared across the call stack frames
     *
     *params:
...
void write_to_file(NODE *root, FILE *new_file, FILE *org_file, int len, char *codes[len]) {
  ...
  * */
  ...
  //have to also append the map of chars to bits at the top of this new file
  //for decoding
...
int main() {
  ...
  write_to_file(root, new_file, file, len, codes); 
